...
extern CKImageViewApp theApp;

void Cmd1Exec(int cmdid, void* pVoid)
{
	CUI* pUI = (CUI*)pVoid;
	
	AfxMessageBox(_T("btn1 exec."));
}
...
int	CUI::CreateMainWnd(HWND hParent)
{
	if(0 == m_client)
	{
		...
		m_client = new CClientView();
		m_client->Create(AfxRegisterWndClass(0), _T(""), WS_CHILD | WS_VISIBLE | WS_BORDER, rc, CWnd::FromHandle(hParent), 101, 0);
	}
	return 1;
}
...
{
	IKToolbar* toolbar = pWnd->CreateToolbar(&theApp, L"Test1", L"aa", L"", L"");
